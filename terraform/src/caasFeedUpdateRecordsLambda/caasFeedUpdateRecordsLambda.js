      ...
      const recordsToUploadSettled = await Promise.allSettled(
        uniqueRecordsToBatchProcess.map(async (incomingUpdateData) => {
          ...
          if (tableRecord?.Items.length > 0) {
            ...
            console.error(
              `Error: Cannot update record as it doesn't exist in table`
            );
 ...
 * @param {Object} incomingUpdateData - The incoming update data.
 * @param {Object} populationTableRecord - The record from the population table.
 * @returns {Object} The response from processing the data.
 */
export const processingData = async (
  ...
  // These fields need to be supplied as we delete the old record and replace it whenever we "update" the record.
  ...
  if (
    ...
    if (retainingPopulationTableRecord.Items.length) {
      ...
      if (
        ...
        if (
          retainingEpisodeRecord.Items.length &&
          supersedingEpisodeRecord.Items.length
        ) {
        ...
        return await mergeRecords(
          incomingUpdateData,
          populationTableRecord,
          retainingPopulationTableRecord
        );
      } else {
        console.error(
          `Error: Rejecting record with supersed NHS number: ${populationTableRecord.nhs_number.N} `
        );
        return {
      ...
      console.error(
        `Error: Superseded Number ${populationTableRecord.nhs_number.N} present in update but record does not exist in our table. Alert to third line support `
      );
      return {
        rejectedRecordNhsNumber: populationTableRecord.nhs_number,
        rejected: true,
 ...
 * @param {Object} incomingUpdateData - The incoming update data.
 * @param {Object} populationTableRecord - The record from the population table.
 * @param {Object} retainingPopulationTableRecord - The record from the retaining population table.
 * @returns {Object} The response from merging the records.
 */
const mergeRecords = async (
  ...
  if (
    overWriteResponse.every((element) => element.value == SUCCESSFUL_RESPONSE)
  ) {
    return {
      rejected: false,
    };
    ...
    console.error(
      `Error: Rejecting record with supersed NHS number: ${populationTableRecord.nhs_number.N}. Investigate these records `
    );
    return {
 ...
 * @param {Object} recordFromTable - The record from the table.
 * @returns {Object} The response from updating the record.
 */
...
function formatPopulationPutItem(table, newRecord) {
  ...
  return {
    Item: {
      ...
      Invited: { S: newRecord.Invited },
      identified_to_be_invited: {
        BOOL: Boolean(newRecord.identified_to_be_invited),
      },
      Batch_Id: { S: newRecord.Batch_Id },
      created_by: { S: newRecord.created_by },
    },
    TableName: `${ENVIRONMENT}-${table}`,
  };
}
 ...
 * @returns {Array<Array>} An array containing unique records array and an array of duplicates.
 */
