...
describe("extractParticipantIds", () => {
  test("return array", async () => {
    ...
    expect(ids.includes(records[0].dynamodb.NewImage.Participant_Id.S)).toBe(
      true
    );
    expect(ids.includes(records[1].dynamodb.NewImage.Participant_Id.S)).toBe(
      true
    );
    expect(ids.includes(records[0].dynamodb.NewImage.Participant_Id.S)).toBe(
      true
...
describe("getInvitationBatch", () => {
  ...
  test("return batch", async () => {
    ...
    const ids = ["id1", "id2"];
    const batchItem = {
      nhsNumber: item.nhs_number.N,
      dateOfBirth: item.date_of_birth.S,
    };
    mockDynamoDbClient.on(QueryCommand).resolves({
      $metadata: {
        httpStatusCode: 200,
...
describe("pushJsonToS3", () => {
  ...
  test("json pushed to s3", async () => {
    ...
    const batchItem = {
      participantId: "NHS-AA11-AA11",
      nhsNumber: "123",
      dateOfBirth: "2000-01-01",
    };
    const jsonArr = [batchItem];
    const result = await pushJsonToS3(
      mockS3Client,
      "bucketName",
      "key",
      jsonArr
