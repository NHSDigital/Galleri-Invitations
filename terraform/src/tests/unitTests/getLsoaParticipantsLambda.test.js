import {
  queryEligiblePopulation,
  getPopulation,
} from "../../getLsoaParticipantsLambda/getLsoaParticipantsLambda";
...
describe("queryEligiblePopulation", () => {
  ...
  test("should push items to array on successful response", async () => {
    ...
    const result = await queryEligiblePopulation(
      mockDynamoDbClient,
      lsoaCode,
      tableItems
    );
...
describe("getPopulation", () => {
  ...
  test("should loop through and add property of LSOA with population info", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      `lsoa being queried number 2. Population object has 0`
    );
  });
});
...
describe("getEligiblePopulation", () => {
  ...
  test("If eligible return pushed popArr", async () => {
    ...
    mockDynamoDbClient.resolves({
      ...
      body: [
        { personId: "9000149009", imdDecile: "5", forecastUptake: "25" },
    ...
    expect(logSpy).toHaveBeenCalled();
    expect(logSpy).toHaveBeenCalledWith(
      `lsoa being queried number 1. Population object has 0`
    );
  });
});
