...
export const saveArrayToTable = async(dataArray, environment, client) => {
  ...
  return Promise.all(
    ...
    const lsoa = await getItemFromTable(item["Postcode"], environment, client);
          const params = {
              "Key": {
                  "gp_practice_code": {
                      "S": item["Code"]
              ...
              "UpdateExpression": "set #ICB = :icb, #NAME = :name, #ADDRESS1 = :address1, #ADDRESS2 = :address2, #ADDRESS3 = :address3, #ADDRESS4 = :address4, #ADDRESS5 = :address5, #POSTCODE = :postcode, #TELEPHONE = :telephone, #OPEN = :start, #CLOSE = :close, #LSOA = :lsoa, #UPDATED = :updated",
          };
